...
export function MetabolicStateCard() {
    ...
    return (
      <div className="bg-dark-card border border-dark-border rounded-2xl p-6">
        <div className="animate-pulse space-y-4">
          <div className="h-6 bg-dark-hover rounded w-1/2"></div>
    ...
    return (
      <div className="bg-dark-card border border-dark-border rounded-2xl p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-dark-text">Estado Metabólico</p>
  ...
  return (
    <div className="bg-dark-card border border-dark-border rounded-2xl p-6">
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-dark-muted">{metabolicState.label}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-8 mb-6">
        <div className="relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <div className="text-3xl font-bold text-dark-text">{hours}</div>
        ...
        <div className="flex-1 space-y-3">
          ...
          <p className="text-sm text-dark-muted leading-relaxed">{metabolicState.description}</p>
          <div className="flex items-center gap-4 text-xs">
            <div>
              <p className="text-dark-muted">Fase</p>
              <p className="font-bold text-dark-text">{metabolicState.state.replace('_', ' ')}</p>
            </div>
            {metabolicState.hasHighLeucine !== undefined && (
              <div>
                <p className="text-dark-muted">Leucina</p>
                <p className={`font-bold ${metabolicState.hasHighLeucine ? 'text-green-500' : 'text-dark-muted'}`}>
      ...
      <div className="p-4 rounded-xl border bg-dark-hover border-dark-border">
        <div className="flex items-center gap-2 mb-2">
          {metabolicState.state === 'mTOR_ACTIVE' && <Zap className="w-4 h-4 text-primary" />}
          {metabolicState.state === 'NEUTRAL' && <Shield className="w-4 h-4 text-dark-muted" />}
      ...
      {metabolicState.state === 'mTOR_ACTIVE' && (
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div className="p-3 rounded-lg bg-dark-hover border border-dark-border">
            ...
            <p className="text-sm font-bold text-dark-text">Activo</p>
          </div>
          <div className="p-3 rounded-lg bg-dark-hover border border-dark-border">
            <div className="flex items-center gap-2 mb-1">
              <Flame className={`w-3 h-3 ${
                metabolicState.insulinLevel === 'high' ? 'text-red-500' :
                metabolicState.insulinLevel === 'moderate' ? 'text-dark-muted' :
                'text-dark-muted'
              }`} />
      ...
      {metabolicState.state === 'NEUTRAL' && metabolicState.insulinLevel && (
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div className="p-3 rounded-lg bg-dark-hover border border-dark-border">
            <div className="flex items-center gap-2 mb-1">
              <Shield className="w-3 h-3 text-dark-muted" />
              <p className="text-xs text-dark-muted">Balance</p>
            </div>
            <p className="text-sm font-bold text-dark-text">Estable</p>
          </div>
          <div className="p-3 rounded-lg bg-dark-hover border border-dark-border">
            <div className="flex items-center gap-2 mb-1">
              <Flame className="w-3 h-3 text-dark-muted" />
              <p className="text-xs text-dark-muted">Insulina</p>
            </div>
      ...
      {(metabolicState.state === 'AUTOPHAGY_EARLY' || metabolicState.state === 'AUTOPHAGY_DEEP') && (
        <div className="mt-4 grid grid-cols-2 gap-3">
          ...
          <div className="p-3 rounded-lg bg-dark-hover border border-dark-border">
            ...
            <p className="text-sm font-bold text-dark-text">Óptima</p>
          </div>
        </div>
      )}
    </div>
